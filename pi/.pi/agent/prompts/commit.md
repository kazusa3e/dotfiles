...
Notes:
- **Header format is `<type>(<scope>) : <subject>`** — a single space on *both* sides of the colon, not the standard `type(scope): subject`. Allowed types: `feat`, `fix`, `refact`, `docs`, `chore`, `perf`, `test`, `build`, `ci`, `style`.
